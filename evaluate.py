...
def evaluate(
    ...
    # Load the model 
    model, config = load_model_from_config(config_file, det_thresh=0.05)  # We set low det_thresh here to measure AP
    ...
    # Filter dataset
    test_dataset = dataset.query("split == @split")
    ...
    evaluation = MIDOGEvaluation(
        ...
        preds=preds,
        output_file=Path(config_file).with_suffix('.json'),
